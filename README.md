...
envarg.add_argument("--model_id", help="Model id name to be loaded.")
...
envarg.add_argument("--need_Resize", type=bool, default=False, help="need to resize pic?.")
...
def scale_image_with_crop_padding(image, shapes,imgName):
    ...
    return image_croped, shapes,imgName

def _Resize_function(image_decoded,shapes,imgName):
    # image_string = tf.read_file(filename)
    # image_decoded = tf.image.decode_image(image_string)
    image_resized = tf.image.resize_images(image_decoded, [input_args.ResizeHeight,input_args.ResizeWidth])
    print("Func image_resized = ",image_resized)
    return image_resized,shapes,imgName
...
def tf_record_parser(record):
    ...
    imgName = tf.cast(features['img_Name'], tf.string)
    height = tf.cast(features['height'], tf.int32)
    width = tf.cast(features['width'], tf.int32)

    # reshape input and annotation images
    image = tf.reshape(image, (height, width, 3), name="image_reshape")
    return tf.to_float(image), (height, width),imgName
...
def create_tfrecord_dataset(Test_images_dir,filename_list, writer):
    ...
    for i, image_name in enumerate(filename_list):
        ...
        example = tf.train.Example(features=tf.train.Features(feature={
            ...
            'img_Name':_bytes_feature(bytes(image_name,encoding='utf-8')),
...
def draw_label2(label, img, label_names,save_path,colormap=None):
    if colormap is None:
        max_index_lab = np.max(label)+1
        cmap = label_colormap(max_index_lab)
        print("np.max(label)+1=",max_index_lab)
        cmap = np.array(cmap * 255).astype(np.uint8)
    else:
        cmap = np.array(colormap, dtype=np.uint8)
    lbl_viz = cmap[label]
    lbl_viz[label == -1] = (0, 0, 0)
    res = img * 0.7 + lbl_viz * 0.3
...
if __name__ == '__main__':
    ...
    print ("need_Resize +++++ = ",input_args.need_Resize)
    if input_args.need_Resize:
        print("**************************")
        test_dataset=test_dataset.map(_Resize_function)
    test_dataset = test_dataset.batch(input_args.batch_size)

    iterator = test_dataset.make_one_shot_iterator()
    batch_images_tf, batch_shapes_tf, img_Name_tf = iterator.get_next()
    ...
    with tf.Session() as sess:
        ...
        while True:
            try:
                ...
                batch_images_np, batch_predictions_np, batch_shapes_np, batch_imgName_np = sess.run(
                    [batch_images_tf, predictions_tf, batch_shapes_tf, img_Name_tf])
                heights, widths = batch_shapes_np
                sessend_time = time.time()-start_time
                ...
                for i in range(batch_predictions_np.shape[0]):
                    print("index/batch_predictions_np.shape[0] = {}/{}   {}  s/sess".format(index, batch_predictions_np.shape[0],sessend_time))
                    ...
                    print("pred_image.shape[0]*pred_image[1] = {}*{}={}".format(pred_image.shape[0],pred_image.shape[1],sizeofShape))
                    print("input_image.shape[0] = ",input_image.shape[0])
                    #print("batch_imgName_np = ", batch_imgName_np)
                    reshap_start_time = time.time()
                    if sizeofShape == input_args.ResizeWidth * input_args.ResizeHeight:
                        print("Resized!!!pred_image_h={} pred_image_w={} input_image_h={} input_image_w={}"
                        .format(pred_image.shape[0],pred_image.shape[1],input_image.shape[0],input_image.shape[1]))
                        #pred_image = np.reshape(pred_image, (heights[i], widths[i]))
                        #pred_image = sess.run(tf.image.resize_images(pred_image, [input_args.ResizeHeight,input_args.ResizeWidth]))
                        #input_image = sess.run(tf.image.resize_images(input_image, [input_args.ResizeHeight,input_args.ResizeWidth]))
                        pred_image = np.reshape(pred_image, (input_args.ResizeHeight, input_args.ResizeWidth))
                        input_image = np.reshape(input_image, (input_args.ResizeHeight, input_args.ResizeWidth, 3))
                    else:
                        print("Doesn't need reshape! W={} H={}".format(widths[i],heights[i]))
                        print("Doesn't resized!!!pred_image_h={} pred_image_w={} input_image_h={} input_image_w={}"
                        .format(pred_image.shape[0],pred_image.shape[1],input_image.shape[0],input_image.shape[1]))
                        #pred_image = np.reshape(pred_image, (heights[i], widths[i]))
                        #input_image = np.reshape(input_image, (heights[i], widths[i], 3))
                    reshap_end_time = time.time()

                    if (input_args.save_Result):
                        cls_dict = {0:'background',1:'jd_bill',2:'bicycle',3:'bird',4:'boat',5:'bottle',6:'bus',7:'car',8:'cat',
                        ...
                        save_path = os.path.join(save_dir, str(batch_imgName_np[i])+"_mask_" + str(index) + ".png")
                        #draw_label(pred_image, input_image, captions,save_path)
                        palette_base = [i for i in range(0, 256, int(255 / 3))]
                        palette = [(palette_base[i], palette_base[j] , palette_base[k]) for i in range(4) for j in range(4) for k in range(4)]
                        palette = [[0,0,0],[0,0,255],[0,255,0],[0,0,255]]
                        draw_label2(pred_image, input_image, captions,save_path,colormap=palette)
                        index += 1
                        print("shape_time:{} s/iter    save_time:{} s/iter".format(reshap_end_time-reshap_start_time,time.time()-start_time))
